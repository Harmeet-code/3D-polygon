...
window.AUDIT = {
  ...
  auditBoothTransformation: function (boothNo) {
    ...
    if (mesh) {
      ...
      if (userData.center) {
        console.log(
          `  Stored center: [${userData.center.x.toFixed(3)}, ${userData.center.y.toFixed(3)}, ${userData.center.z.toFixed(3)}]`
        );
      }
      mesh.geometry.computeBoundingBox();
      const box = mesh.geometry.boundingBox;
      console.log(
        `  Geometry X/Z center: [${((box.max.x + box.min.x) / 2).toFixed(4)}, ${((box.max.z + box.min.z) / 2).toFixed(4)}]`
      );
      console.log(`  Geometry Y extent: [${box.min.y.toFixed(4)}, ${box.max.y.toFixed(4)}]`);
      console.log(
        '  Geometry should be centered on X/Z with its bottom face above the floor plane'
      );
    }
  ...
  auditAllBoothCentering: function () {
    ...
    boothMeshes.forEach((mesh) => {
      ...
      if (Math.abs(cx) > tolerance || Math.abs(cz) > tolerance || box.min.y < -tolerance) {
        console.warn(
          `${mesh.userData.booth.boothNo}: geometry not placed correctly! centerXZ=[${cx.toFixed(4)}, ${cz.toFixed(4)}], yMin=${box.min.y.toFixed(4)}`
        );
  ...
  runAll: function () {
    ...
    console.log(
      '%c🔍 COMPLETE COORDINATE SYSTEM AUDIT',
      'color: #6aa9ff; font-weight: bold; font-size: 14px;'
    );
    console.log('%c═══════════════════════════════════════', 'color: #6aa9ff; font-weight: bold;');
...
function buildBooths() {
  ...
  for (const b of data.booths) {
    ...
    bb.getCenter(c); // Get center point in 3D space
    const minY = bb.min.y;
...
const focusBtn = /** @type {HTMLButtonElement} */ (document.getElementById('focusBtn'));
...
/* ------------------------------ */
/* DROPDOWNS */
const fromSelect = /** @type {HTMLSelectElement} */ (document.getElementById('fromSelect'));
const toSelect = /** @type {HTMLSelectElement} */ (document.getElementById('toSelect'));
...
const markerVideo = /** @type {HTMLVideoElement} */ (document.getElementById('markerVideo'));
