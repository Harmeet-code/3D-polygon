...
function demoCompany(no){
  const names=["Acme Corp","BluePeak","Cedar & Co","Nova Labs","Orbit Systems","PineWorks","Zenith AI","RiverStone"];
  return names[hashStr(no)%names.length];
}
function demoPrice(no, base=1000){ return base + (hashStr(no)%5)*250; }
...
const floorTex = await new Promise((res, rej)=> new THREE.TextureLoader().load(encodeURI(data.meta.image), res, undefined, rej));
...
window.DEBUG = {
  ...
  checkBoothPolygon: function(boothNo) {
    ...
    if(!b) { console.error("Booth not found:", boothNo); return; }
    ...
    const calcW = maxX - minX, calcH = maxY - minY;
  ...
  traceTransform: function(boothNo) {
    ...
    console.log("3️⃣ World Coords:", worldPt, `(plane is ${PLANE_W.toFixed(1)}x${PLANE_H.toFixed(1)})`);
  ...
  compareBooths: function(boothNos = ["P18", "P19", "P66", "NE4"]) {
    ...
    boothNos.forEach(no => {
      ...
      console.log(`${no}: fabric[${pt[0]},${pt[1]}] → pixel[${pix.px.toFixed(0)},${pix.py.toFixed(0)}] → world[${world.x.toFixed(2)},${world.z.toFixed(2)}]`);
    });
    console.groupEnd();
  },
  ...
  showImageInfo: function() {
    ...
    const info = {
      ...
      calibration: cal,
      effectiveScale: {
        x: baseScaleX * cal.scaleX,
        y: baseScaleY * cal.scaleY
      },
      rendererCanvas: {
  ...
  // Show the projected pixel/world box for one booth
  getBoothBox: function(boothNo) {
    const b = data.booths.find(x => x.boothNo === boothNo);
    if(!b) { console.error("Booth not found:", boothNo); return null; }
    ...
    const wx = worlds.map(p => p.x);
    const wz = worlds.map(p => p.z);
    const box = {
      boothNo,
      pixel: {
        minX: Math.min(...xs),
  ...
  checkImageBounds: function(boothNos = ["T1", "T2", "P18", "NE4"]) {
    const rows = boothNos.map(no => {
      ...
      return {
        ...
        insideImage: box.pixel.minX >= 0 && box.pixel.maxX <= IMG_W && box.pixel.minY >= 0 && box.pixel.maxY <= IMG_H
      };
    }).filter(Boolean);
    console.table(rows);
    return rows;
  ...
  showFabricImageMapping: function() {
    const cal = readCal();
    const left = cal.offsetX;
    const right = (fb.maxX - fb.minX) * baseScaleX * cal.scaleX + cal.offsetX;
    const top = cal.offsetY;
    const bottom = (fb.maxY - fb.minY) * baseScaleY * cal.scaleY + cal.offsetY;
    const mapping = {
      image: { width: IMG_W, height: IMG_H },
      mappedFabricRect: { left, right, top, bottom, width: right - left, height: bottom - top },
      marginsPx: { left, right: IMG_W - right, top, bottom: IMG_H - bottom },
      calibration: cal
    };
...
/* ------------------------------ */
/* COORDINATE SYSTEM AUDIT & VALIDATION */
window.AUDIT = {
  // Verify center-at-origin for floor plane
  auditFloorPlane: function() {
    ...
    console.log("  min:", { x: box.min.x.toFixed(2), y: box.min.y.toFixed(2), z: box.min.z.toFixed(2) });
    console.log("  max:", { x: box.max.x.toFixed(2), y: box.max.y.toFixed(2), z: box.max.z.toFixed(2) });
    console.log("  center:", { 
  ...
  // Verify coordinate transformations are mathematically correct
  auditCoordinateTransforms: function() {
    ...
    const expectedScaleX = IMG_W / (fb.maxX - fb.minX);
    const expectedScaleY = IMG_H / (fb.maxY - fb.minY);
    console.log("Scale calculation verify:");
    console.log(`  ${IMG_W} / ${(fb.maxX - fb.minX).toFixed(2)} = ${expectedScaleX.toFixed(6)} ✓`);
    console.log(`  ${IMG_H} / ${(fb.maxY - fb.minY).toFixed(2)} = ${expectedScaleY.toFixed(6)} ✓`);
    
    console.groupEnd();
  },
  
  // Trace complete transformation for a booth
  auditBoothTransformation: function(boothNo) {
    const b = data.booths.find(x => x.boothNo === boothNo);
    if (!b) { console.error("Booth not found:", boothNo); return; }
    ...
    const pts = b.geometry.points;
    console.log("All corner transformations:");
    ...
    pts.forEach((pt, idx) => {
      ...
      console.group(`  Corner ${idx + 1}`);
      console.log(`Fabric:  [${fabricPt[0].toFixed(2)}, ${fabricPt[1].toFixed(2)}]`);
      console.log(`Pixel:   [${pixPt.px.toFixed(1)}, ${pixPt.py.toFixed(1)}]`);
      console.log(`World:   [${worldPt.x.toFixed(3)}, ${worldPt.z.toFixed(3)}]`);
      console.groupEnd();
    });
    
    // Verify booth mesh centering
    ...
    if (mesh) {
      ...
      if (userData.center) {
        console.log(`  Stored center: [${userData.center.x.toFixed(3)}, ${userData.center.y.toFixed(3)}, ${userData.center.z.toFixed(3)}]`);
      }
      mesh.geometry.computeBoundingBox();
      const box = mesh.geometry.boundingBox;
      console.log(`  Geometry X/Z center: [${((box.max.x + box.min.x) / 2).toFixed(4)}, ${((box.max.z + box.min.z) / 2).toFixed(4)}]`);
      console.log(`  Geometry Y extent: [${box.min.y.toFixed(4)}, ${box.max.y.toFixed(4)}]`);
      console.log("  Geometry should be centered on X/Z with its bottom face above the floor plane");
    }
    
    console.groupEnd();
  },
  
  // Verify all booths are centered on X/Z and sit above the floor plane
  auditAllBoothCentering: function() {
    ...
    boothMeshes.forEach(mesh => {
      ...
      if (Math.abs(cx) > tolerance || Math.abs(cz) > tolerance || box.min.y < -tolerance) {
        console.warn(`${mesh.userData.booth.boothNo}: geometry not placed correctly! centerXZ=[${cx.toFixed(4)}, ${cz.toFixed(4)}], yMin=${box.min.y.toFixed(4)}`);
        centersCorrect = false;
      }
    });
    
    if (centersCorrect) {
      console.log(`All ${boothMeshes.length} booths are centered on X/Z and sit above the floor plane`);
    }
    console.groupEnd();
  },
  
  // Check for winding order issues
  auditWindingOrder: function() {
    ...
    data.booths.forEach(b => {
      ...
      // Convert to 3D world and check again
      const pix = pts.map(pt => fabricToPixel(pt[0], pt[1]));
      const world = pix.map(p => pxToWorld(p.px, p.py));
      const world2d = world.map(w => ({x: w.x, y: w.z}));
      
      let area3d = 0;
      for (let i = 0; i < world2d.length; i++) {
        const p = world2d[i], q = world2d[(i + 1) % world2d.length];
        area3d += (p.x * q.y - q.x * p.y);
      }
      area3d /= 2;
      
      // Image Y points down while world Z points up, so the sign should flip.
      if (Math.sign(area) === Math.sign(area3d)) {
        console.warn(`${b.boothNo}: winding sign did not flip as expected during image-to-world transform`);
        windingIssues++;
      }
    });
  ...
  // Run all audits
  runAll: function() {
    console.log("%c═══════════════════════════════════════", "color: #6aa9ff; font-weight: bold;");
    console.log("%c🔍 COMPLETE COORDINATE SYSTEM AUDIT", "color: #6aa9ff; font-weight: bold; font-size: 14px;");
    console.log("%c═══════════════════════════════════════", "color: #6aa9ff; font-weight: bold;");
    ...
    console.log("%c═══════════════════════════════════════", "color: #6aa9ff; font-weight: bold;");
    console.log("%c✓ All systems nominal!", "color: #2ecc71; font-weight: bold; font-size: 14px;");
    console.log("%cTry: AUDIT.auditBoothTransformation('P18')", "color: #6aa9ff;");
    console.log("%c═══════════════════════════════════════", "color: #6aa9ff; font-weight: bold;");
  }
};
...
function heatColor(t){
  ...
  let r,g,b;
  if(t < 0.25){
    const k=t/0.25; r=0; g=lerp(80,220,k); b=255;
  } else if(t < 0.5){
    const k=(t-0.25)/0.25; r=0; g=255; b=lerp(255,60,k);
  } else if(t < 0.75){
    const k=(t-0.5)/0.25; r=lerp(0,255,k); g=255; b=0;
  } else {
    const k=(t-0.75)/0.25; r=255; g=lerp(255,60,k); b=0;
  }
  return (r<<16) | (g<<8) | (b<<0);
}
...
/* ------------------------------ */
/* BOOTHS */
const BOOTH_FLOOR_GAP = 0.06;
const boothGroup = new THREE.Group();
boothGroup.position.y = BOOTH_FLOOR_GAP;
scene.add(boothGroup);

const outlineGroup = new THREE.Group();
outlineGroup.position.y = BOOTH_FLOOR_GAP + 0.05;
...
function buildBooths(){
  ...
  for(const b of data.booths){
    /* ═══════════════════════════════════════════════════════════════ */
    /* STEP 1-2: COORDINATE TRANSFORMATION CHAIN                       */
    /* Fabric (design units) → Pixel (image coords) → 3D World (scene) */
    /* ═══════════════════════════════════════════════════════════════ */
    
    // Step 1a: Get booth corners in fabric coordinates (from JSON)
    // These are abstract design units, not pixels or 3D coordinates
    const ptsPix = b.geometry.points.map(([x,y]) => fabricToPixel(x,y));
    
    // Step 1b: Convert pixel coordinates to 3D world space on the floor plane
    // The floor plane is 140×93.33 units, centered at (0,0,0)
    // This creates 2D Vector2 coords on the XZ plane (Y=0 is ground level)
    const pts2 = ptsPix.map(p => {
      const w = pxToWorld(p.px, p.py);
      return new THREE.Vector2(w.x, w.z);
    });

    // Skip invalid polygons (less than 3 points can't form a shape)
    if(pts2.length < 3) continue;
    
    // Ensure consistent winding order (counterclockwise when viewed from above)
    // This matters for face orientation and lighting
    if(polygonArea(pts2) < 0) pts2.reverse();

    /* ═══════════════════════════════════════════════════════════════ */
    /* STEP 3: CREATE 3D EXTRUDED GEOMETRY                             */
    /* ═══════════════════════════════════════════════════════════════ */
    
    const shape = new THREE.Shape(pts2);
    
    // Height varies by booth status (visual distinction)
    const h = (b.status==="BOOKED") ? 2.0 : (b.status==="HOLD" ? 1.6 : 1.2);

    // ExtrudeGeometry creates a 3D solid by extruding the 2D shape
    // It extrudes along the Z-axis (which will be rotated to Y-axis below)
    const geo = new THREE.ExtrudeGeometry(shape, { depth: h, bevelEnabled:false, steps:1 });
    
    // Rotate so extrusion lies flat on ground
    // Before: Shape in XY plane, extruded along Z → stands up
    // After: Shape now on XZ plane, sits at Y=0 (ground level)
    geo.rotateX(-Math.PI/2);

    /* ═══════════════════════════════════════════════════════════════ */
    /* STEP 4: CENTER GEOMETRY AT LOCAL ORIGIN (CRITICAL!)             */
    /* ═══════════════════════════════════════════════════════════════ */
    
    // By default, ExtrudeGeometry creates vertices at arbitrary positions
    // We need all vertices centered at the geometry's local (0,0,0) so that:
    // - mesh.position exactly represents the booth center
    // - raycasting/picking works from the center
    // - highlighting/scaling works symmetrically
    // - shadows and lighting are correct
    
    geo.computeBoundingBox();  // Calculate min/max extents
    const c = new THREE.Vector3();
    geo.boundingBox.getCenter(c);  // Get center point in 3D space
    const minY = geo.boundingBox.min.y;
    
    // Center X/Z only, then put the bottom face at local Y=0.
    // Centering Y would put half the booth below the floor image plane.
    geo.translate(-c.x, -minY, -c.z);
    
    // After this, all vertices are centered: min ≈ -width/2, max ≈ +width/2

    /* ═══════════════════════════════════════════════════════════════ */
    /* STEP 5: CREATE MESH AND POSITION IN SCENE                       */
    /* ═══════════════════════════════════════════════════════════════ */
    
    // Create mesh with centered geometry
    const mesh = new THREE.Mesh(geo, boothMaterialFor(b));
    
    // Position mesh in scene at the point where geometry's X/Z center was.
    // Y=0 because the booth group supplies the small lift above the floor.
    mesh.position.set(c.x, 0, c.z);
    
    // The combination of centered geometry + positioned mesh means:
    // - Geometry vertices are centered on X/Z and start at local Y=0
    // - Mesh.position is at (c.x, 0, c.z)
    // - Booth appears at world position (c.x, 0, c.z) ✓

    mesh.castShadow = true;
    mesh.receiveShadow = true;
    mesh.userData.booth = b;

    // Store center for later use (camera focus, routing, tooltips)
...
function updateSidebar(b){
  ...
  statusDot.style.background =
    (b.status==="AVAILABLE") ? "var(--good)" :
    (b.status==="BOOKED") ? "var(--bad)" : "var(--warn)";
...
focusBtn.addEventListener("click", ()=>{ if(selected) focusMesh(selected); });
...
function applyFilters(){
  ...
  boothMeshes.forEach(m=>{
    ...
    const matchText = !q || b.boothNo.toLowerCase().includes(q) || ((b.company||"").toLowerCase().includes(q));
...
function aStar(start, goal){
  ...
  const dirs = [
    ...
    {dr:1,dc:1,cost:1.4},{dr:1,dc:-1,cost:1.4},{dr:-1,dc:1,cost:1.4},{dr:-1,dc:-1,cost:1.4}
  ];
  ...
  while(open.size){
    ...
    if(cur.i === gIdx){
      ...
      let at=gIdx;
      while(at !== -1){
        const rr=Math.floor(at/cols), cc=at%cols;
        path.push({r:rr,c:cc});
        at = came[at];
      }
      path.reverse();
...
// Route visuals
let routeBase=null, routeGlow=null, routeDots=null;
...
function clearRoute(){
  ...
  if(routeBase){ scene.remove(routeBase); routeBase.geometry.dispose(); routeBase.material.dispose(); routeBase=null; }
  if(routeGlow){ scene.remove(routeGlow); routeGlow.geometry.dispose(); routeGlow.material.dispose(); routeGlow=null; }
  if(routeDots){ scene.remove(routeDots); routeDots.geometry.dispose(); routeDots.material.dispose(); routeDots=null; }
}
...
function drawRoute(worldPoints){
  ...
  const baseGeom = new THREE.TubeGeometry(curve, 220, 0.50, 10, false);
  const baseMat = new THREE.MeshStandardMaterial({ color:0x07101f, roughness:0.6, metalness:0.2, transparent:true, opacity:0.92 });
  ...
  const glowMat = new THREE.MeshBasicMaterial({ color:0x6aa9ff, transparent:true, opacity:0.55 });
  ...
  const dotsMat = new THREE.PointsMaterial({ color:0xffffff, size:0.55, transparent:true, opacity:0.85 });
...
document.getElementById("followCam").addEventListener("change", ()=>{
  if(document.getElementById("followCam").checked && routeWorldPoints) followRoute(routeWorldPoints);
  else clearFollow();
});

/* ------------------------------ */
/* CAMERA HUD */
document.getElementById("camIso").addEventListener("click", ()=> flyTo(new THREE.Vector3(75,70,75), new THREE.Vector3(0,0,0), 850));
document.getElementById("camTop").addEventListener("click", ()=> flyTo(new THREE.Vector3(0,160,0.01), new THREE.Vector3(0,0,0), 850));
document.getElementById("camReset").addEventListener("click", ()=> flyTo(new THREE.Vector3(70,70,90), new THREE.Vector3(0,0,0), 850));
...
document.getElementById("tour").addEventListener("click", ()=>{
  ...
  if(!touring){ clearInterval(tourTimer); tourTimer=null; return; }
  clearRoute();
  let i=0;
...
/* ------------------------------ */
/* CALIBRATION HOOKS */
function onCalChange(){ persistCal(); buildBooths(); }
...
document.getElementById("resetCal").addEventListener("click", ()=>{
  ...
  offXEl.value = DEFAULT_CALIBRATION.offsetX;
  offYEl.value = DEFAULT_CALIBRATION.offsetY;
...
console.log(
  "%c🔍 DEBUG UTILITIES\n" +
  "Debugging:\n" +
  "  DEBUG.showImageInfo()           // Image & scaling info\n" +
  "  DEBUG.traceTransform('P18')     // Trace coords for a booth\n" +
  "  DEBUG.checkBoothPolygon('P18')  // Verify booth geometry\n" +
  "  DEBUG.compareBooths()           // Compare multiple booths\n" +
  "\n🔍 AUDIT UTILITIES (Verify coordinate system)\n" +
  "  AUDIT.runAll()                  // Run complete audit\n" +
  "  AUDIT.auditFloorPlane()         // Check floor centering\n" +
  "  AUDIT.auditBoothTransformation('P18') // Trace single booth\n" +
  "  AUDIT.auditAllBoothCentering()  // Check all booth centering",
...
function tick(){
  ...
  if(routeGlow){
    ...
    routeGlow.material.opacity = 0.20 + 0.55*(0.5 + 0.5*Math.sin(t));
  }
  if(routeDots){
